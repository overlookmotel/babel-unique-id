...
module.exports = function(state, options) {
	// Get plugin name from options
	const pluginName = conformPluginName(options);

	// Prefix error messages with plugin name
	try {
		return createId(state, options);
	} catch (err) {
		if (err && pluginName) err.message = `${pluginName}: ${err.message || 'Unknown error'}`;
		throw err;
	}
};

function createId(state, options) {
	// Conform options
...
function conformPluginName(options) {
	const {pluginName} = options;
	if (pluginName == null) return undefined;

	invariant(
		isFullString(pluginName),
...
function conformOptions(options) {
	...
	if (counterKey === undefined) {
		...
		invariant(
			isSymbol(counterKey),
			`options.counterKey must be a Symbol if provided - got ${counterKey}`
		);
	}
...
function validatePackage(packageProps) {
